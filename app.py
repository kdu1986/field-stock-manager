...
def carregar_dados(arquivo, padrao=[]):
    try:
        ...
        with open(arquivo, 'r', encoding='utf-8-sig') as f:
            dados = json.load(f)
            # Normalização de categorias para o padrão Procer
            for d in dados:
...
with col_logo:
    try:
        # CORREÇÃO: Removido o '=' solto que causava erro de sintaxe
...
if dados:
    ...
    # Alertas Críticos de Estoque (SOC Mindset: Monitoramento Ativo)
    ...
    if auth:
        if st.button("Inicializar Estoque"):
